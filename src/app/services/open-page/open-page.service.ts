...
export class OpenPageService {
  ...
  constructor(private inAppBrowser: InAppBrowser, private alertCtrl: AlertController, private toastCtrl: ToastController) {}
  ...
  this.subscription.add(this.browser.on('loadstop').subscribe(() => {this.hideLoading().then(() => this.browser.show());
}));
